...
save_address='performance_vs_rounds\CL_without_sampling';

% learning rate
eta=0.1;

% repeat time (Monte Carlo)
repeat_times=1;
...
% network initialization and training options
...
%     gradient_aggregated=get_aggregated_gradient(K,Q,gradient_FL,N,N_r,load_address,rd);
    % Do not consider the impact of wireless channels
...
%     global_net.b{2}=reshape(global_net_new_vector(start_point+1:start_point+size(vec(global_net.b{2}),1)),size(global_net.b{2}));
    ...
    % compute the loss value of cross-entropy
...
% save the accuracy and loss values obtained in each round
